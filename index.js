...
// This tells the bot WHERE to find images for each category.
// You can change these names if you want different sources.
const SUBREDDIT_MAP = {
    'girl': 'RealGirls',
    'boy': 'LadyBoners',
    'hentai': 'hentai',
...
async function fetchFromReddit(category) {
    ...
    try {
        // Fetch Top 60 Hot posts from Reddit JSON API
    ...
    new SlashCommandBuilder()
        ...
        .addSubcommand(sub => 
            sub.setName('refill')
               ...
               .addStringOption(o => o.setName('category').setRequired(true).addChoices(...VALID_CATEGORIES.map(c => ({name: c, value: c}))))
        )
        .addSubcommand(sub => 
            sub.setName('reset')
...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'refill') {
        ...
        if (added === -1) {
            ...
            statusMsg.edit("⚠️ No *new* images found right now (or duplicates). Try again tomorrow!");
        } else {
            const data = loadData();
